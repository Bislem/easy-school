...
const money = (value: any) =>
    `${Number(value).toLocaleString('fr-FR', { minimumFractionDigits: 2 })} ${props.currency.symbol}`;
...
function save() {
    ...
    if (editingId.value)
        form.put(`/admin/salaries/configurations/${editingId.value}`, options);
    else form.post('/admin/salaries/configurations', options);
}
async function remove(configuration: any) {
    if (
        await appConfirm(
            `Supprimer la configuration « ${configuration.name} » ?`,
            {
        ...
        router.delete(`/admin/salaries/configurations/${configuration.id}`, {
            preserveScroll: true,
        });
}
</script>
...
<template>
    ...
    <AdminLayout
            ...
            <div class="mx-auto max-w-6xl space-y-6">
                ...
                >
                    <div>
                        ...
                        >
                            <Settings class="size-6" />Paramètres de paie
                        </h1>
                        <p class="text-sm text-muted-foreground">
                            Gérez les modes et taux de calcul disponibles pour
                            les bulletins.
                        </p>
                    </div>
                ...
                <section class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                    ...
                    >
                        <div class="flex items-start justify-between gap-3">
                            <div>
                                ...
                                <p class="text-sm text-muted-foreground">
                                    {{ labels[configuration.salary_type] }}
                                </p>
                            </div>
                            <span
                        ...
                        <p class="mt-5 text-2xl font-semibold">
                            {{ money(configuration.base_rate) }}
                        </p>
                        <p class="mt-1 text-xs text-muted-foreground">
                            Valable du {{ configuration.effective_from }} au
                            {{
                                configuration.effective_to || 'sans date de fin'
                            }}
                        </p>
                        <p
                            v-if="configuration.notes"
                        ...
                        >
                            <span class="text-xs text-muted-foreground"
                                >{{
                                    configuration.statements_count
                                }}
                                bulletin(s)</span
                            >
                            <div class="flex gap-1">
                                ...
                                ><Button
                                    ...
                                    :disabled="
                                        configuration.statements_count > 0
                                    "
                                    :title="
                                        configuration.statements_count > 0
                                            ? 'Configuration utilisée : définissez plutôt une date de fin'
                                            : 'Supprimer'
                                    "
                    ...
                    <div
                        ...
                        class="col-span-full rounded-xl border border-dashed bg-card p-12 text-center"
                    >
                        <Settings
                            class="mx-auto mb-3 size-8 text-muted-foreground"
                        /><b>Aucune configuration salariale</b>
                        <p class="mt-1 text-sm text-muted-foreground">
                            Créez la première configuration pour commencer à
        ...
        >
            <form
                class="mx-auto my-10 max-w-xl space-y-4 rounded-xl bg-background p-6 shadow-xl"
                @submit.prevent="save"
            >
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-sm text-muted-foreground">
                            Définissez le mode de rémunération et sa période de
                            validité.
                        </p>
                    </div>
                    <Button
                ...
                <label class="block"
                    ...
                    ><Input
                        v-model="form.name"
                        required
                        placeholder="Ex. Formateur horaire standard" /><InputError
                ...
                ><label class="block"
                    ...
                    ><InputError :message="form.errors.salary_type" /></label
                ><label class="block"
                ...
                <div class="grid gap-3 sm:grid-cols-2">
                    <label
                        ...
                        ><Input
                            ...
                            :message="form.errors.effective_from" /></label
                    ><label
                ...
                <label class="block"
                    ...
                    ></textarea
                    ><InputError :message="form.errors.notes" /></label
