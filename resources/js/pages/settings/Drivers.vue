...
function submit() {
    const url = editingId.value
        ? `/settings/drivers/${editingId.value}`
        : '/settings/drivers';
    form.post(url, {
        forceFormData: true,
        preserveScroll: true,
...
async function remove(driver: Driver) {
    if (
        await appConfirm(`Supprimer ${driver.full_name} de vos conducteurs ?`, {
            title: 'Supprimer le conducteur',
            tone: 'danger',
            confirmText: 'Supprimer',
        })
    ) {
        useForm({}).delete(`/settings/drivers/${driver.id}`, {
            preserveScroll: true,
        });
    }
}
...
<template>
    <AppLayout>
        ...
        <SettingsLayout>
            <div class="space-y-6">
                <HeadingSmall
                    title="Conducteurs supplémentaires"
                    description="Enregistrez jusqu’à 3 conducteurs. Seuls les conducteurs approuvés peuvent être affectés à une réservation."
                />
                            ...
                            <div>
                                <div class="flex items-center gap-2">
                                    ...
                                    <span
                                        ...
                                        >{{
                                            statusLabel(driver.approval_status)
                                        }}</span
                                    >
                                </div>
                                ...
                                <a
                                    ...
                                    target="_blank"
                                    class="mt-2 inline-block text-sm text-orange-600 underline"
                                    >Voir le permis</a
                                >
                                <p
                ...
                >
                    ...
                    <p v-if="editingId" class="text-sm text-amber-700">
                        Toute modification nécessite une nouvelle approbation.
                    </p>
                    <div>
                ...
                <p v-else class="rounded-lg bg-muted p-4 text-sm">
                    Vous avez atteint la limite de
                    {{ maximumDrivers }} conducteurs.
                </p>
            </div>
        </SettingsLayout>
    </AppLayout>
