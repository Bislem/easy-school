...
const opened = ref<number | null>(props.statements.data[0]?.id ?? null);
const labels: any = {
...
const money = (v: any) =>
    `${Number(v || 0).toLocaleString('fr-DZ', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ${props.currency}`;
const hours = (s: any) =>
    Number(s.calculation_details?.attendance_worked_hours ?? 0);
const monthLabel = (v: string) =>
    new Intl.DateTimeFormat('fr-FR', { month: 'long', year: 'numeric' }).format(
        new Date(`${v}-01T00:00:00`),
    );
const date = (v: string) => new Date(v).toLocaleDateString('fr-FR');
const method = (v: string) =>
    (
        ({
            cash: 'Espèces',
        ...
        ><main class="min-w-0 flex-1 p-4 sm:p-6 lg:p-8">
            <div class="mx-auto max-w-6xl space-y-6">
                <header class="rounded-2xl border bg-card p-5 shadow-sm sm:p-6">
                    <div class="flex items-center justify-between gap-4">
                        <div>
                            ...
                            <h1 class="mt-1 text-2xl font-bold">Ma paie</h1>
                ...
                <section class="grid grid-cols-2 gap-3 lg:grid-cols-5">
                    <article
                        v-for="x in [
                            ...
                            {
                                l: 'Payé',
                                v: money(summary.paid),
                                i: CheckCircle2,
                                c: 'text-emerald-600 bg-emerald-100',
                            },
                            {
                                l: 'Reste',
                                v: money(summary.remaining),
                                i: Banknote,
                                c: 'text-amber-600 bg-amber-100',
                            },
                        ]"
                        :key="x.l"
                        class="rounded-xl border bg-card p-4 shadow-sm last:col-span-2 lg:last:col-span-1"
                    >
                        <span class="inline-grid rounded-lg p-2" :class="x.c"
                            ><component :is="x.i" class="size-4"
                        /></span>
                        <p class="mt-3 text-xs text-muted-foreground">
                            {{ x.l }}
                        </p>
                        <b class="mt-1 block text-lg break-words">{{ x.v }}</b>
                    </article>
                </section>
                    ...
                    <div class="flex flex-col gap-3 sm:flex-row sm:items-end">
                        ...
                        ><Button @click="filter"
                            ><CalendarDays
                                class="mr-2 size-4"
                            />Afficher</Button
                        ><Button v-if="period" variant="ghost" @click="clear"
                            ><RotateCcw
                                class="mr-2 size-4"
                ...
                <section class="space-y-4">
                    ...
                    >
                        <button
                            class="flex w-full flex-col gap-4 p-5 text-left sm:flex-row sm:items-center sm:justify-between"
                            @click="opened = opened === s.id ? null : s.id"
                        >
                            <div>
                            ...
                            >
                                ...
                                <ChevronDown
                                    ...
                                    :class="opened === s.id ? 'rotate-180' : ''"
                                />
                            </div>
                        </button>
                        <div
                            v-if="opened === s.id"
                            class="border-t bg-muted/10 p-4 sm:p-5"
                        >
                            <div class="grid gap-4 lg:grid-cols-3">
                                ...
                                >
                                    ...
                                    <Button
                                        ...
                                        class="mt-4 w-full"
                                        ><a
                                            :href="`/my/salary/statements/${s.id}/download`"
                                            ><Download
                                                class="mr-2 size-4"
                                            />Bulletin PDF</a
                                        ></Button
                            ...
                            <div
                                v-if="s.teacher_attendances?.length"
                                class="mt-4 rounded-xl border bg-background p-4"
                            >
                                <p class="font-semibold">Séances incluses</p>
                                <div class="mt-2 divide-y">
                                    <div
                                        v-for="a in s.teacher_attendances"
                                        :key="a.id"
                                        class="grid gap-1 py-3 text-sm sm:grid-cols-[110px_1fr_auto]"
                                    >
                                        <span>{{
                                            date(a.session.starts_at)
                                        }}</span
                                        ><span
                                            ><b>{{ a.session.title }}</b
                                            ><small
                            ...
                            <div
                                v-if="s.adjustments?.length"
                                class="mt-4 rounded-xl border bg-background p-4"
                            >
                                <p class="font-semibold">
                                    Primes et retenues détaillées
                                </p>
                                <div
                            ...
                            <div
                                class="mt-4 rounded-xl border bg-background p-4"
                            >
                                <p class="font-semibold">Paiements et reçus</p>
                                <div
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        <Button
                                            ...
                                            variant="outline"
                                            ><a
                                                :href="`/my/salary/payments/${p.id}/receipt`"
                                                ><ReceiptText
                                                    class="mr-2 size-4"
                                                />Reçu</a
                                            ></Button
