...
test('an administrator can create a plan from an enrollment form with all its groups', function () {
    ...
    $this->actingAs($admin)->post(route('admin.training-plans.store'), [
        'source_type' => 'form', 'enrollment_form_id' => $form->id,
    ...
    expect($plan->groups()->count())->toBe(3)
        ->and($plan->course_id)->toBe($course->id)
...
test('room conflicts and excessive planned duration are rejected', function () {
    ...
    $room = Classroom::create(['name' => 'Salle A', 'code' => 'SA-PLAN', 'capacity' => 20, 'is_active' => true]);
    $otherRoom = Classroom::create(['name' => 'Salle B', 'code' => 'SB-PLAN', 'capacity' => 20, 'is_active' => true]);
    $plan = TrainingPlan::create(['course_id' => $course->id, 'teacher_id' => $teacher->id, 'title' => 'Plan Excel', 'status' => 'draft']);
