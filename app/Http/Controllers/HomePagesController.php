...
class HomePagesController extends Controller
{
    ...
    {
        $bookableStatuses = [CarStatus::AVAILABLE->value, CarStatus::RESERVED->value, CarStatus::RENTED->value];

        $query = Car::whereIn('status', $bookableStatuses)
        ...
        // Get filter options
        $makes = Car::whereIn('status', $bookableStatuses)
            ->distinct()
            ->pluck('make')
            ->toArray();

        $fuelTypes = Car::whereIn('status', $bookableStatuses)
            ->distinct()
            ->pluck('fuel_type')
            ->toArray();

        $years = Car::whereIn('status', $bookableStatuses)
            ->distinct()
            ->pluck('year')
            ->toArray();
