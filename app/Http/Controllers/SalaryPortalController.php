...
class SalaryPortalController extends Controller
{
    ...
    public function statement(Request $request,SalaryStatement $statement):HttpResponse{$staff=$this->staff($request);abort_unless($statement->staff_id===$staff->id,403);$statement->load(['staff.employeeType','payments','adjustments','lines','statutoryLines','regulation','configuration']);return Pdf::loadView('admin.salaries.statement',['statement'=>$statement,'school'=>CompanySetting::current(),'currency'=>config('app.currency_symbol')])->download($statement->reference.'.pdf');}
