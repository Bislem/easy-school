...
class PortalController extends Controller
{
 public function dashboard(Request $request): Response
 {
  ...
  $sessions=$this->teacherSessions($user->id);$groups=$this->teacherGroups($user->id);$students=$this->enrollmentsForGroups($groups)->pluck('student')->filter()->unique('id')->values();
  ...
  return Inertia::render('Portal/Dashboard',['kind'=>$user->staff?->is_teacher?'teacher':'employee','profile'=>$user->staff,'today'=>$sessions->whereBetween('starts_at',[today()->startOfDay(),today()->endOfDay()])->values(),'upcoming'=>$sessions->where('starts_at','>',now())->take(12)->values(),'groups'=>$groups,'students'=>$students,'notifications'=>$notifications,'summary'=>['completed_hours'=>round($hours,2)]]);
 }

 public function section(Request $request,string $section): Response
 {
  abort_unless(in_array($section,['formation','planning','attendance','payments','documents','notifications','groups','students'],true),404);$user=$request->user();
  ...
  abort_unless($user->staff,403);$sessions=$this->teacherSessions($user->id);$groups=$this->teacherGroups($user->id);$students=$this->enrollmentsForGroups($groups);
  return Inertia::render('Portal/Section',['kind'=>$user->staff->is_teacher?'teacher':'employee','section'=>$section,'data'=>['sessions'=>$sessions,'groups'=>$groups,'students'=>$students,'notifications'=>$user->portalNotifications()->get()]]);
 }
 ...
 private function teacherSessions(int $userId){return TrainingSession::with(['group.plan.course:id,title','group.enrollments.student','classroom:id,name','attendances'])->where('teacher_id',$userId)->orderBy('starts_at')->get();}
 private function teacherGroups(int $userId){return TrainingPlanGroup::with(['plan.course','sessions.classroom'])->where(fn($q)=>$q->whereHas('plan',fn($q)=>$q->where('teacher_id',$userId))->orWhereHas('sessions',fn($q)=>$q->where('teacher_id',$userId)))->get();}
