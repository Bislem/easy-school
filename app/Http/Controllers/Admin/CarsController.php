...
class CarsController extends Controller
{
    ...
    {
        $car->load([
            'fuelTankRecords' => fn($query) => $query
                ->with(['reservation:id,reservation_number', 'recordedBy:id,name'])
                ->latest('recorded_at'),
        ...
        $reservations = $car->reservations()
            ...
            ->withQueryString();
        // dd(CarColor::forFrontend());

        return Inertia::render('Admin/Cars/Edit', [
            'car' => $car,
            'imageFiles' => $imageFiles,
            'reservations' => $reservations,
            'reservationStatuses' => ReservationStatus::getMeta(),
            'enums' => [
                'colors' => CarColor::forFrontend(),
                // 'fuelTypes' => FuelType::forFrontend(),
                'fuelTypes' => FuelType::forFrontend(),
    ...
    public function update(Request $request, Car $car)
    {
        $validated = $request->validate([
            ...
            'license_plate' => [
                ...
                'string',
                'max:255',
                Rule::unique('cars', 'license_plate')->ignore($car->id),
            ],
            'color' => ['required', 'string', Rule::enum(CarColor::class)],
            'price_per_day' => ['required', 'numeric', 'min:0'],
            'security_deposit' => ['required', 'numeric', 'min:0'],
