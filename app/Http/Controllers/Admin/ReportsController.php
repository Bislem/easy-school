...
class ReportsController extends Controller
{
    ...
    private function getHighLevelKPIs(array $dateRange): array
    {
        ...
        return [
            ...
            'totalExpenses' => [
                'value' => $totalExpenses,
                'formatted' => config('app.currency_symbol') . number_format($totalExpenses, 2),
                'label' => 'Dépenses totales'
            ],
            'netProfit' => [
                'value' => $netProfit,
                'formatted' => config('app.currency_symbol') . number_format($netProfit, 2),
                'label' => 'Résultat net'
            ],
            'platformVisits' => [
    ...
    private function getReservationsChart(array $dateRange): array
    {
        ...
        $statusLabels = collect(ReservationStatus::getMeta())
    ...
    private function getCarsPerformance(array $dateRange)
    {
        $carsPerformance = Car::withCount(['reservations as total_reservations' => function ($query) use ($dateRange) {
            ...
            ->withSum(['expenses as total_expenses' => function ($query) use ($dateRange) {
            ...
            ->map(function ($car) {
                ...
                $totalExpenses = (float) ($car->total_expenses ?? 0);
                ...
                return [
                    ...
                    'formatted_expenses' => config('app.currency_symbol') . number_format($totalExpenses, 2),
                    'net_profit' => $totalRevenue - $totalExpenses,
                    'formatted_net_profit' => config('app.currency_symbol') . number_format($totalRevenue - $totalExpenses, 2),
