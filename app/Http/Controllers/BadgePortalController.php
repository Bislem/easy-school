...
class BadgePortalController extends Controller
{
    ...
    public function verify(string $token): Response { $badge=Badge::where('verification_token',$token)->firstOrFail();app(TenantContext::class)->set($badge->tenant_id);return Inertia::render('Badge/Verify',['badge'=>$badge->only(['card_number','first_name','last_name','person_type','role_label','formation_label','group_label','issue_date','expiration_date','display_status']),'school'=>CompanySetting::current()->only(['trading_name','logo_url'])]); }
